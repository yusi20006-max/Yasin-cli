class HubAdapter {
  ...
  ensureRegistered() {
    ...
    if (!services[this.serviceId]) {
      this.serviceManager.registerService(
        ...
        'YasinHub Service',
        'node',
        ['-e', 'setInterval(() => console.log("YasinHub running..."), 5000);']
      );
    }
  }
  ...
  doctor() {
    ...
    const checks = [
      { name: 'Hub Service Registered', status: 'PASS' },
      { name: 'Hub Service State', status: status.status === 'running' ? 'PASS' : 'WARN' }
    ];
    return {
