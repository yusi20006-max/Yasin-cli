...
class StatusCommand extends Command {
  constructor(configManager, serviceManager = null, pluginSystem = null, statusOperation = null) {
    super({
      name: 'status',
  ...
  execute(args, options = {}) {
    ...
    if (!data.services.initialized) {
      ...
      data.services.items.forEach(s => console.log(`  - [${s.status.toUpperCase()}] ${s.id} (PID: ${s.pid || 'N/A'})`));
    }
    console.log();
    ...
    if (!data.plugins.initialized) {
      ...
      data.plugins.items.forEach(p => console.log(`  - [${p.enabled ? 'ENABLED' : 'DISABLED'}] ${p.id} v${p.version || '1.0.0'}`));
    }
