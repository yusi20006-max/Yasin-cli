...
class AgentCommand extends Command {
  constructor(adapter) {
    super({ name: 'agent', description: 'Manage Yasin-Agent on-demand execution', args: [{ name: 'action', required: true, description: 'status, doctor, run, version, or config' }, { name: 'subAction', required: false }, { name: 'key', required: false }, { name: 'value', required: false }], options: [{ name: '--json', type: 'boolean', default: false, description: 'Emit machine-readable JSON' }], supportsJson: true });
  ...
  execute(args, options = {}) {
    ...
    } else throw new Error(`Unsupported Yasin-Agent action "${action}". Use run, status, doctor, version, or config.`);
    if (options.json) return AutomationResult.success(data);
    if (action === 'run') process.stdout.write(data.stdout || 'Yasin-Agent completed successfully.\n');
