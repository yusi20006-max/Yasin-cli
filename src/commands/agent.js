...
class AgentCommand extends Command {
  constructor(adapter) {
    super({
      name: 'agent',
      description: 'Manage and monitor Yasin-Agent ecosystem components',
      args: [
  ...
  execute(args, options) {
    ...
    if (action === 'doctor') {
      ...
      console.log(`=== Yasin-Agent Diagnostics ===\n`);
    ...
    if (action === 'start') {
      try {
        ...
        console.log(`Yasin-Agent started successfully with PID ${details.pid}.`);
      } catch (err) {
        console.error(`Error starting Yasin-Agent: ${err.message}`);
        process.exit(1);
      }
      return;
    }
    ...
    if (action === 'stop') {
      ...
      if (success) {
        console.log(`Yasin-Agent stopped successfully.`);
      } else {
        console.log(`Yasin-Agent was not running or not found.`);
      }
      return;
    }

    if (action === 'restart') {
      console.log(`Restarting Yasin-Agent...`);
      try {
        const details = this.adapter.restart();
        console.log(`Yasin-Agent restarted successfully with PID ${details.pid}.`);
      } catch (err) {
        console.error(`Error restarting Yasin-Agent: ${err.message}`);
        process.exit(1);
      }
      return;
    }
    ...
    if (action === 'version') {
      ...
      console.log(`Yasin-Agent version: ${version}`);
      return;
    }
