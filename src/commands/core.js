...
class CoreCommand extends Command {
  constructor(adapter) {
    super({
      name: 'core',
      description: 'Manage and monitor Yasin-Core ecosystem components',
      args: [
  ...
  execute(args, options) {
    ...
    if (action === 'doctor') {
      ...
      console.log(`=== Yasin-Core Diagnostics ===\n`);
    ...
    if (action === 'start') {
      try {
        ...
        console.log(`Yasin-Core started successfully with PID ${details.pid}.`);
      } catch (err) {
        console.error(`Error starting Yasin-Core: ${err.message}`);
        process.exit(1);
      }
      return;
    }
    ...
    if (action === 'stop') {
      ...
      if (success) {
        console.log(`Yasin-Core stopped successfully.`);
      } else {
        console.log(`Yasin-Core was not running or not found.`);
      }
      return;
    }

    if (action === 'restart') {
      console.log(`Restarting Yasin-Core...`);
      try {
        const details = this.adapter.restart();
        console.log(`Yasin-Core restarted successfully with PID ${details.pid}.`);
      } catch (err) {
        console.error(`Error restarting Yasin-Core: ${err.message}`);
        process.exit(1);
      }
      return;
    }
    ...
    if (action === 'version') {
      ...
      console.log(`Yasin-Core version: ${version}`);
      return;
    }
