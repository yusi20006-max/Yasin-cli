...
class RelayCommand extends Command {
  constructor(adapter) {
    super({ name: 'relay', description: 'Manage YasinRelay background execution', args: [{ name: 'action', required: true, description: 'status, doctor, start, stop, restart, version, or config' }, { name: 'subAction', required: false }, { name: 'key', required: false }, { name: 'value', required: false }] });
  ...
  execute(args) {
    ...
    if (action === 'start') { const details = this.adapter.start(); console.log(`YasinRelay started successfully with PID ${details.pid}.`); return; }
    if (action === 'stop') { console.log(this.adapter.stop() ? 'YasinRelay stopped successfully.' : 'YasinRelay was not running or not found.'); return; }
    if (action === 'restart') { const details = this.adapter.restart(); console.log(`YasinRelay restarted successfully with PID ${details.pid}.`); return; }
    ...
    throw new Error(`Unsupported YasinRelay action "${action}".`);
  }
}
