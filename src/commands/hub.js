...
class HubCommand extends Command {
  constructor(adapter) {
    super({
      name: 'hub',
      description: 'Manage and monitor YasinHub ecosystem components',
      args: [
  ...
  execute(args, options) {
    ...
    if (action === 'doctor') {
      ...
      console.log(`=== YasinHub Diagnostics ===\n`);
    ...
    if (action === 'start') {
      try {
        ...
        console.log(`YasinHub started successfully with PID ${details.pid}.`);
      } catch (err) {
        console.error(`Error starting YasinHub: ${err.message}`);
        process.exit(1);
      }
      return;
    }
    ...
    if (action === 'stop') {
      ...
      if (success) {
        console.log(`YasinHub stopped successfully.`);
      } else {
        console.log(`YasinHub was not running or not found.`);
      }
      return;
    }

    if (action === 'restart') {
      console.log(`Restarting YasinHub...`);
      try {
        const details = this.adapter.restart();
        console.log(`YasinHub restarted successfully with PID ${details.pid}.`);
      } catch (err) {
        console.error(`Error restarting YasinHub: ${err.message}`);
        process.exit(1);
      }
      return;
    }
    ...
    if (action === 'version') {
      ...
      console.log(`YasinHub version: ${version}`);
      return;
    }
