...
class HubCommand extends Command {
  constructor(adapter) {
    super({ name: 'hub', description: 'Manage YasinHub on-demand status', args: [{ name: 'action', required: true, description: 'status, doctor, run, version, or config' }, { name: 'subAction', required: false }, { name: 'key', required: false }, { name: 'value', required: false }] });
  ...
  execute(args) {
    ...
    if (action === 'run') {
      ...
      process.stdout.write(result.stdout || 'YasinHub completed successfully.\n');
      return;
    }
    if (action === 'config') {
    ...
    throw new Error(`Unsupported YasinHub action "${action}". Use run, status, doctor, version, or config.`);
  }
}
